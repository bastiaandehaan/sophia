...
class EMAStrategy:
    ...
    def check_signals(self, symbol: str, data: Optional[pd.DataFrame] = None) -> Dict[
        ...
        if data is None:
            ...
            bars_needed = max(self.slow_ema,
                              self.rsi_period) + 30  # Extra bars voor goede berekening
        ...
        indicators = {"current_price": data["close"].iloc[-1],
            ...
            "slow_ema": data["slow_ema"].iloc[-1], "macd": data["macd"].iloc[-1],
            "signal": data["signal"].iloc[-1], "macd_hist": data["macd_hist"].iloc[-1],
            "prev_macd_hist": data["macd_hist"].iloc[-2], "rsi": data["rsi"].iloc[-1],
            "atr": data["atr"].iloc[-1], "momentum": data["momentum"].iloc[-1],
    ...
    def _generate_signal(self, symbol: str, data: pd.DataFrame,
        ...
        if current_direction is None:
            ...
            if (
                    fast_ema > slow_ema and macd > signal_line and macd_hist > 0 and prev_macd_hist <= 0 and  # Crossover
                    rsi > 50 and momentum > 0 and current_price > bollinger_mid):

                signal = "BUY"
                entry_price = current_price
                stop_loss = entry_price - (self.atr_multiplier * atr_value)
                meta = {"entry_price": entry_price, "stop_loss": stop_loss,
                    "reason": "ema_macd_long_entry", "atr": atr_value, }
            ...
            # 4. Prijs onder midden Bollinger (extra filter)
                ...
                stop_loss = entry_price + (self.atr_multiplier * atr_value)
                meta = {"entry_price": entry_price, "stop_loss": stop_loss,
                    "reason": "ema_macd_short_entry", "atr": atr_value, }
        ...
        # Exit logica - voor bestaande posities
            ...
            if (
                    macd < signal_line and macd_hist < 0 and prev_macd_hist >= 0) or fast_ema < slow_ema:
                signal = "CLOSE_BUY"
        ...
        if signal in ["BUY", "SELL"]:
            ...
            try:
                order_result = self.connector.place_order(symbol, signal, position_size,
                    entry_price, stop_loss, take_profit, f"Sophia EMA {signal}")

                if order_result and order_result.get("success"):
                    # Update positie tracking
                    self.positions[symbol] = {"direction": signal,
                        "entry_price": entry_price, "stop_loss": stop_loss,
                        "take_profit": take_profit, "size": position_size,
