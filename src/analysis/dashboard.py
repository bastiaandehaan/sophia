...
class SophiaDashboard:
    ...
    def load_config(self):
        ...
        try:
            ...
            messagebox.showwarning("Configuratie laden",
                                   f"Kon configuratie niet laden: {e}")
            return {}
    ...
    def save_config(self, config):
        ...
        try:
            ...
            messagebox.showerror("Configuratie opslaan",
                                 f"Kon configuratie niet opslaan: {e}")
    ...
    def setup_backtest_tab(self):
        ...
        # Strategie selectie
        ttk.Label(input_frame, text="Strategie:").grid(row=0, column=0, sticky=tk.W,
                                                       padx=5, pady=5)
        self.backtest_strategy_var = tk.StringVar(value="turtle")
        strategy_combobox = ttk.Combobox(input_frame,
                                         textvariable=self.backtest_strategy_var,
                                         values=["turtle", "ema"], state="readonly")
        strategy_combobox.grid(row=0, column=1, sticky=tk.W + tk.E, padx=5, pady=5)
        strategy_combobox.bind("<<ComboboxSelected>>", self.on_backtest_strategy_change)

        # Symbolen
        ttk.Label(input_frame, text="Symbolen:").grid(row=1, column=0, sticky=tk.W,
                                                      padx=5, pady=5)
        symbols = self.config.get("symbols", ["EURUSD", "USDJPY"])
        self.backtest_symbols_var = tk.StringVar(value=", ".join(symbols))
        ttk.Entry(input_frame, textvariable=self.backtest_symbols_var).grid(row=1,
            column=1, sticky=tk.W + tk.E, padx=5, pady=5)

        # Timeframe
        ttk.Label(input_frame, text="Timeframe:").grid(row=2, column=0, sticky=tk.W,
                                                       padx=5, pady=5)
        self.backtest_timeframe_var = tk.StringVar(
            value=self.config.get("timeframe", "H4"))
        ttk.Combobox(input_frame, textvariable=self.backtest_timeframe_var,
                     values=["M1", "M5", "M15", "M30", "H1", "H4", "D1"],
                     state="readonly").grid(row=2, column=1, sticky=tk.W + tk.E, padx=5,
                                            pady=5)

        # Periode
        ttk.Label(input_frame, text="Periode:").grid(row=3, column=0, sticky=tk.W,
                                                     padx=5, pady=5)
        self.backtest_period_var = tk.StringVar(value="1y")
        ttk.Combobox(input_frame, textvariable=self.backtest_period_var,
                     values=["1m", "3m", "6m", "1y", "2y", "5y"],
                     state="readonly").grid(row=3, column=1, sticky=tk.W + tk.E, padx=5,
                                            pady=5)

        # Initieel kapitaal
        ttk.Label(input_frame, text="Initieel kapitaal:").grid(row=4, column=0,
                                                               sticky=tk.W, padx=5,
                                                               pady=5)
        self.backtest_cash_var = tk.StringVar(value="10000")
        ttk.Entry(input_frame, textvariable=self.backtest_cash_var).grid(row=4,
            column=1, sticky=tk.W + tk.E, padx=5, pady=5)

        # Strategie parameters frame
        strat_frame = ttk.LabelFrame(main_frame, text="Strategie Parameters")
        strat_frame.pack(side=tk.RIGHT, fill=tk.BOTH, expand=True, padx=5, pady=5)
        ...
        # Turtle parameters
        ttk.Label(self.turtle_params_frame, text="Entry Period:").grid(row=0, column=0,
                                                                       sticky=tk.W,
                                                                       padx=5, pady=5)
        self.backtest_entry_period_var = tk.StringVar(value="20")
        ttk.Entry(self.turtle_params_frame,
                  textvariable=self.backtest_entry_period_var).grid(row=0, column=1,
            sticky=tk.W + tk.E, padx=5, pady=5)

        ttk.Label(self.turtle_params_frame, text="Exit Period:").grid(row=1, column=0,
                                                                      sticky=tk.W,
                                                                      padx=5, pady=5)
        self.backtest_exit_period_var = tk.StringVar(value="10")
        ttk.Entry(self.turtle_params_frame,
                  textvariable=self.backtest_exit_period_var).grid(row=1, column=1,
            sticky=tk.W + tk.E, padx=5, pady=5)

        ttk.Label(self.turtle_params_frame, text="ATR Period:").grid(row=2, column=0,
                                                                     sticky=tk.W,
                                                                     padx=5, pady=5)
        self.backtest_atr_period_var = tk.StringVar(value="14")
        ttk.Entry(self.turtle_params_frame,
                  textvariable=self.backtest_atr_period_var).grid(row=2, column=1,
            sticky=tk.W + tk.E, padx=5, pady=5)

        # EMA parameters
        ttk.Label(self.ema_params_frame, text="Fast EMA:").grid(row=0, column=0,
                                                                sticky=tk.W, padx=5,
                                                                pady=5)
        self.backtest_fast_ema_var = tk.StringVar(value="9")
        ttk.Entry(self.ema_params_frame, textvariable=self.backtest_fast_ema_var).grid(
            row=0, column=1, sticky=tk.W + tk.E, padx=5, pady=5)

        ttk.Label(self.ema_params_frame, text="Slow EMA:").grid(row=1, column=0,
                                                                sticky=tk.W, padx=5,
                                                                pady=5)
        self.backtest_slow_ema_var = tk.StringVar(value="21")
        ttk.Entry(self.ema_params_frame, textvariable=self.backtest_slow_ema_var).grid(
            row=1, column=1, sticky=tk.W + tk.E, padx=5, pady=5)

        ttk.Label(self.ema_params_frame, text="Signal EMA:").grid(row=2, column=0,
                                                                  sticky=tk.W, padx=5,
                                                                  pady=5)
        self.backtest_signal_ema_var = tk.StringVar(value="5")
        ttk.Entry(self.ema_params_frame,
                  textvariable=self.backtest_signal_ema_var).grid(row=2, column=1,
            sticky=tk.W + tk.E, padx=5, pady=5)
        ...
        ttk.Checkbutton(options_frame, text="Plot resultaten",
                        variable=self.backtest_plot_var).pack(side=tk.LEFT, padx=5,
            pady=5)
        ...
        self.backtest_output = tk.Text(output_frame, height=10,
        ...
        ttk.Button(button_frame, text="Start Backtest", command=self.run_backtest).pack(
            side=tk.RIGHT, padx=5)

        ttk.Button(button_frame, text="Open Resultaten Map",
                   command=lambda: self.open_folder("backtest_results")).pack(
            side=tk.RIGHT, padx=5)
    ...
    def setup_optimize_tab(self):
        ...
        # Input frame links
        input_frame = ttk.LabelFrame(main_frame, text="Optimalisatie Parameters")
        input_frame.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=5, pady=5)
        ...
        strategy_combobox = ttk.Combobox(input_frame,
                                         ...
                                         values=["turtle", "ema"], state="readonly")
        strategy_combobox.grid(row=0, column=1, sticky=tk.W + tk.E, padx=5, pady=5)
        strategy_combobox.bind("<<ComboboxSelected>>", self.on_optimize_strategy_change)
        ...
        ttk.Entry(input_frame, textvariable=self.optimize_symbols_var).grid(row=1,
            column=1, sticky=tk.W + tk.E, padx=5, pady=5)
        ...
        self.optimize_timeframe_var = tk.StringVar(
            value=self.config.get("timeframe", "H4"))
        ttk.Combobox(input_frame, textvariable=self.optimize_timeframe_var,
                     values=["M1", "M5", "M15", "M30", "H1", "H4", "D1"],
                     state="readonly").grid(row=2, column=1, sticky=tk.W + tk.E, padx=5,
                                            pady=5)
        ...
        ttk.Combobox(input_frame, textvariable=self.optimize_period_var,
                     values=["1m", "3m", "6m", "1y", "2y", "5y"],
                     state="readonly").grid(row=3, column=1, sticky=tk.W + tk.E, padx=5,
                                            pady=5)

        # Metric om te optimaliseren
        ttk.Label(input_frame, text="Optimaliseer voor:").grid(row=4, column=0,
                                                               sticky=tk.W, padx=5,
                                                               pady=5)
        self.optimize_metric_var = tk.StringVar(value="sharpe")
        ttk.Combobox(input_frame, textvariable=self.optimize_metric_var,
                     values=["sharpe", "return", "drawdown", "profit_factor"],
                     state="readonly").grid(row=4, column=1, sticky=tk.W + tk.E, padx=5,
                                            pady=5)

        # Maximum combinaties
        ttk.Label(input_frame, text="Max combinaties:").grid(row=5, column=0,
                                                             sticky=tk.W, padx=5,
                                                             pady=5)
        self.optimize_max_combinations_var = tk.StringVar(value="50")
        ttk.Entry(input_frame, textvariable=self.optimize_max_combinations_var).grid(
        ...
        param_frame.pack(side=tk.RIGHT, fill=tk.BOTH, expand=True, padx=5, pady=5)
        ...
        # Turtle parameters
        ttk.Label(self.turtle_opt_frame, text="Entry Period:").grid(row=0, column=0,
                                                                    sticky=tk.W, padx=5,
                                                                    pady=5)
        self.optimize_entry_range_var = tk.StringVar(value="10,20,30,40")
        ttk.Entry(self.turtle_opt_frame,
                  textvariable=self.optimize_entry_range_var).grid(row=0, column=1,
            sticky=tk.W + tk.E, padx=5, pady=5)

        ttk.Label(self.turtle_opt_frame, text="Exit Period:").grid(row=1, column=0,
                                                                   sticky=tk.W, padx=5,
                                                                   pady=5)
        self.optimize_exit_range_var = tk.StringVar(value="5,10,15,20")
        ttk.Entry(self.turtle_opt_frame,
                  textvariable=self.optimize_exit_range_var).grid(row=1, column=1,
            sticky=tk.W + tk.E, padx=5, pady=5)

        ttk.Label(self.turtle_opt_frame, text="ATR Period:").grid(row=2, column=0,
                                                                  sticky=tk.W, padx=5,
                                                                  pady=5)
        self.optimize_atr_range_var = tk.StringVar(value="10,14,20")
        ttk.Entry(self.turtle_opt_frame, textvariable=self.optimize_atr_range_var).grid(
            row=2, column=1, sticky=tk.W + tk.E, padx=5, pady=5)

        # EMA parameters
        ttk.Label(self.ema_opt_frame, text="Fast EMA:").grid(row=0, column=0,
                                                             sticky=tk.W, padx=5,
                                                             pady=5)
        self.optimize_fast_ema_range_var = tk.StringVar(value="5,9,12,15")
        ttk.Entry(self.ema_opt_frame,
                  textvariable=self.optimize_fast_ema_range_var).grid(row=0, column=1,
            sticky=tk.W + tk.E, padx=5, pady=5)

        ttk.Label(self.ema_opt_frame, text="Slow EMA:").grid(row=1, column=0,
                                                             sticky=tk.W, padx=5,
                                                             pady=5)
        self.optimize_slow_ema_range_var = tk.StringVar(value="20,25,30")
        ttk.Entry(self.ema_opt_frame,
                  textvariable=self.optimize_slow_ema_range_var).grid(row=1, column=1,
            sticky=tk.W + tk.E, padx=5, pady=5)

        ttk.Label(self.ema_opt_frame, text="Signal EMA:").grid(row=2, column=0,
                                                               sticky=tk.W, padx=5,
                                                               pady=5)
        self.optimize_signal_ema_range_var = tk.StringVar(value="5,7,9")
        ttk.Entry(self.ema_opt_frame,
                  textvariable=self.optimize_signal_ema_range_var).grid(row=2, column=1,
            sticky=tk.W + tk.E, padx=5, pady=5)

        # Beschrijving labels
        ttk.Label(self.turtle_opt_frame, text="Komma-gescheiden waarden").grid(row=3,
            column=0, columnspan=2, sticky=tk.W, padx=5, pady=5)

        ttk.Label(self.ema_opt_frame, text="Komma-gescheiden waarden").grid(row=3,
            column=0, columnspan=2, sticky=tk.W, padx=5, pady=5)
        ...
        self.optimize_output = tk.Text(output_frame, height=10,
                                       yscrollcommand=scrollbar.set, wrap=tk.WORD,
                                       bg="#f0f0f0")
        ...
        ttk.Button(button_frame, text="Start Optimalisatie",
                   command=self.run_optimization).pack(side=tk.RIGHT, padx=5)

        ttk.Button(button_frame, text="Open Resultaten Map",
                   command=lambda: self.open_folder("optimization_results")).pack(
            side=tk.RIGHT, padx=5)
    ...
    def setup_live_tab(self):
        ...
        ttk.Label(status_frame, text="Status:").grid(row=0, column=0, sticky=tk.W,
                                                     padx=5, pady=5)
        ttk.Label(status_frame, textvariable=self.mt5_status_var).grid(row=0, column=1,
            sticky=tk.W, padx=5, pady=5)

        # Verbindingsbutton
        self.connect_button = ttk.Button(status_frame, text="Verbinden",
                                         command=self.toggle_connection)
        self.connect_button.grid(row=0, column=2, padx=5, pady=5)

        # Account info indicators
        self.account_balance_var = tk.StringVar(value="- €")
        ttk.Label(status_frame, text="Balans:").grid(row=1, column=0, sticky=tk.W,
                                                     padx=5, pady=5)
        ttk.Label(status_frame, textvariable=self.account_balance_var).grid(row=1,
            column=1, sticky=tk.W, padx=5, pady=5)

        self.account_equity_var = tk.StringVar(value="- €")
        ttk.Label(status_frame, text="Equity:").grid(row=2, column=0, sticky=tk.W,
                                                     padx=5, pady=5)
        ttk.Label(status_frame, textvariable=self.account_equity_var).grid(row=2,
            column=1, sticky=tk.W, padx=5, pady=5)
        ...
        # Strategie selectie
        ttk.Label(strategy_frame, text="Actieve strategie:").grid(row=0, column=0,
                                                                  sticky=tk.W, padx=5,
                                                                  pady=5)
        self.live_strategy_var = tk.StringVar(value="turtle")
        ttk.Combobox(strategy_frame, textvariable=self.live_strategy_var,
                     values=["turtle", "ema"], state="readonly").grid(row=0, column=1,
            sticky=tk.W + tk.E, padx=5, pady=5)

        # Symbolen
        ttk.Label(strategy_frame, text="Symbolen:").grid(row=1, column=0, sticky=tk.W,
                                                         padx=5, pady=5)
        symbols = self.config.get("symbols", ["EURUSD", "USDJPY"])
        self.live_symbols_var = tk.StringVar(value=", ".join(symbols))
        ttk.Entry(strategy_frame, textvariable=self.live_symbols_var).grid(row=1,
            column=1, sticky=tk.W + tk.E, padx=5, pady=5)

        # Timeframe
        ttk.Label(strategy_frame, text="Timeframe:").grid(row=2, column=0, sticky=tk.W,
                                                          padx=5, pady=5)
        self.live_timeframe_var = tk.StringVar(value=self.config.get("timeframe", "H4"))
        ttk.Combobox(strategy_frame, textvariable=self.live_timeframe_var,
                     values=["M1", "M5", "M15", "M30", "H1", "H4", "D1"],
                     state="readonly").grid(row=2, column=1, sticky=tk.W + tk.E, padx=5,
                                            pady=5)

        # Risico
        ttk.Label(strategy_frame, text="Risico per trade:").grid(row=3, column=0,
                                                                 sticky=tk.W, padx=5,
                                                                 pady=5)
        ...
        # Interval
        ttk.Label(strategy_frame, text="Check interval:").grid(row=4, column=0,
                                                               sticky=tk.W, padx=5,
                                                               pady=5)
        ...
        ttk.Entry(interval_frame, textvariable=self.live_interval_var, width=5).pack(
            side=tk.LEFT)
        ...
        self.positions_tree = ttk.Treeview(positions_frame,
            columns=("symbol", "direction", "size", "entry", "current", "profit"),
        ...
        self.live_log = tk.Text(log_frame, height=10, yscrollcommand=scrollbar.set,
        ...
        ttk.Button(button_frame, text="Sluit Alle Posities",
                   command=self.close_all_positions).pack(side=tk.RIGHT, padx=5)
    ...
    def setup_config_tab(self):
        ...
        # MT5 pad
        ttk.Label(mt5_frame, text="MT5 pad:").grid(row=0, column=0, sticky=tk.W, padx=5,
                                                   pady=5)
        self.mt5_path_var = tk.StringVar(
            value=self.config.get("mt5", {}).get("mt5_path",
                "C:\\Program Files\\MetaTrader 5\\terminal64.exe"))
        path_frame = ttk.Frame(mt5_frame)
        path_frame.grid(row=0, column=1, sticky=tk.W + tk.E, padx=5, pady=5)

        ttk.Entry(path_frame, textvariable=self.mt5_path_var).pack(side=tk.LEFT,
                                                                   fill=tk.X,
                                                                   expand=True)
        ttk.Button(path_frame, text="Bladeren", command=self.browse_mt5_path).pack(
            side=tk.RIGHT)

        # MT5 login
        ttk.Label(mt5_frame, text="Login:").grid(row=1, column=0, sticky=tk.W, padx=5,
                                                 pady=5)
        self.mt5_login_var = tk.StringVar(
            value=str(self.config.get("mt5", {}).get("login", "")))
        ttk.Entry(mt5_frame, textvariable=self.mt5_login_var).grid(row=1, column=1,
            sticky=tk.W + tk.E, padx=5, pady=5)

        # MT5 wachtwoord
        ttk.Label(mt5_frame, text="Wachtwoord:").grid(row=2, column=0, sticky=tk.W,
                                                      padx=5, pady=5)
        self.mt5_password_var = tk.StringVar(
            value=self.config.get("mt5", {}).get("password", ""))
        ttk.Entry(mt5_frame, textvariable=self.mt5_password_var, show="*").grid(row=2,
            column=1, sticky=tk.W + tk.E, padx=5, pady=5)

        # MT5 server
        ttk.Label(mt5_frame, text="Server:").grid(row=3, column=0, sticky=tk.W, padx=5,
                                                  pady=5)
        self.mt5_server_var = tk.StringVar(
            value=self.config.get("mt5", {}).get("server", ""))
        ttk.Entry(mt5_frame, textvariable=self.mt5_server_var).grid(row=3, column=1,
            sticky=tk.W + tk.E, padx=5, pady=5)
        ...
        # Risico per trade
        ttk.Label(risk_frame, text="Risico per trade (%):").grid(row=0, column=0,
                                                                 sticky=tk.W, padx=5,
                                                                 pady=5)
        self.risk_per_trade_var = tk.StringVar(
            value=str(self.config.get("risk", {}).get("risk_per_trade", 0.01) * 100))
        ttk.Entry(risk_frame, textvariable=self.risk_per_trade_var).grid(row=0,
            column=1, sticky=tk.W + tk.E, padx=5, pady=5)

        # Max dagelijks verlies
        ttk.Label(risk_frame, text="Max dagelijks verlies (%):").grid(row=1, column=0,
                                                                      sticky=tk.W,
                                                                      padx=5, pady=5)
        self.max_daily_loss_var = tk.StringVar(
            value=str(self.config.get("risk", {}).get("max_daily_loss", 0.05) * 100))
        ttk.Entry(risk_frame, textvariable=self.max_daily_loss_var).grid(row=1,
            column=1, sticky=tk.W + tk.E, padx=5, pady=5)

        # Max posities
        ttk.Label(risk_frame, text="Max aantal posities:").grid(row=2, column=0,
                                                                sticky=tk.W, padx=5,
                                                                pady=5)
        self.max_positions_var = tk.StringVar(
            value=str(self.config.get("risk", {}).get("max_positions", 5)))
        ttk.Entry(risk_frame, textvariable=self.max_positions_var).grid(row=2, column=1,
            sticky=tk.W + tk.E, padx=5, pady=5)

        # Max gecorreleerde posities
        ttk.Label(risk_frame, text="Max gecorreleerde posities:").grid(row=3, column=0,
                                                                       sticky=tk.W,
                                                                       padx=5, pady=5)
        self.max_correlated_var = tk.StringVar(
            value=str(self.config.get("risk", {}).get("max_correlated", 2)))
        ttk.Entry(risk_frame, textvariable=self.max_correlated_var).grid(row=3,
            column=1, sticky=tk.W + tk.E, padx=5, pady=5)
        ...
        # Symbolen
        ttk.Label(general_frame, text="Symbolen:").grid(row=0, column=0, sticky=tk.W,
                                                        padx=5, pady=5)
        self.symbols_var = tk.StringVar(
            value=", ".join(self.config.get("symbols", ["EURUSD", "USDJPY"])))
        ttk.Entry(general_frame, textvariable=self.symbols_var).grid(row=0, column=1,
            sticky=tk.W + tk.E, padx=5, pady=5)

        # Timeframe
        ttk.Label(general_frame, text="Timeframe:").grid(row=1, column=0, sticky=tk.W,
                                                         padx=5, pady=5)
        self.timeframe_var = tk.StringVar(value=self.config.get("timeframe", "H4"))
        ttk.Combobox(general_frame, textvariable=self.timeframe_var,
                     values=["M1", "M5", "M15", "M30", "H1", "H4", "D1"],
                     state="readonly").grid(row=1, column=1, sticky=tk.W + tk.E, padx=5,
                                            pady=5)

        # Interval
        ttk.Label(general_frame, text="Check interval (s):").grid(row=2, column=0,
                                                                  sticky=tk.W, padx=5,
                                                                  pady=5)
        self.interval_var = tk.StringVar(value=str(self.config.get("interval", 300)))
        ttk.Entry(general_frame, textvariable=self.interval_var).grid(row=2, column=1,
            sticky=tk.W + tk.E, padx=5, pady=5)
        ...
        ttk.Button(button_frame, text="Opslaan", command=self.save_configuration).pack(
            side=tk.RIGHT, padx=5)

        ttk.Button(button_frame, text="Herstellen",
    ...
    def update_backtest_parameters_frame(self):
        ...
        if strategy == "turtle":
            self.turtle_params_frame.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)
        elif strategy == "ema":
            self.ema_params_frame.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)
    ...
    def update_optimize_parameters_frame(self):
        ...
        # Toon juiste parameters frame
        if strategy == "turtle":
            self.turtle_opt_frame.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)
        elif strategy == "ema":
    ...
    def browse_mt5_path(self):
        ...
        filename = filedialog.askopenfilename(title="Selecteer MetaTrader 5 executable",
            filetypes=[("Executable", "*.exe")], initialdir="C:\\Program Files")
        if filename:
    ...
    def save_configuration(self):
        ...
        try:
            # MT5 configuratie
            mt5_config = {"mt5_path": self.mt5_path_var.get(), "login": int(
                self.mt5_login_var.get()) if self.mt5_login_var.get() else 0,
                "password": self.mt5_password_var.get(),
                "server": self.mt5_server_var.get()}

            # Risico configuratie
            risk_config = {"risk_per_trade": float(self.risk_per_trade_var.get()) / 100,
            ...
            # Symbolen
            symbols = [s.strip() for s in self.symbols_var.get().split(",") if
                       s.strip()]

            # Volledige configuratie
            config = {"mt5": mt5_config, "risk": risk_config, "symbols": symbols,
            ...
            self.config = config
            messagebox.showinfo("Configuratie", "Configuratie succesvol opgeslagen")
    ...
    def reset_configuration(self):
        ...
        # Update UI velden
        # MT5
        self.mt5_path_var.set(config.get("mt5", {}).get("mt5_path",
            "C:\\Program Files\\MetaTrader 5\\terminal64.exe"))
        ...
        self.max_positions_var.set(str(config.get("risk", {}).get("max_positions", 5)))
        self.max_correlated_var.set(
            str(config.get("risk", {}).get("max_correlated", 2)))

        # Algemeen
        self.symbols_var.set(", ".join(config.get("symbols", ["EURUSD", "USDJPY"])))
    ...
    def start_trading(self):
        ...
        self.live_log.insert(tk.END, f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} "
    ...
    def close_all_positions(self):
        ...
        confirm = messagebox.askyesno("Posities sluiten",
    ...
    def run_backtest(self):
        ...
        strategy = self.backtest_strategy_var.get()
        symbols = [s.strip() for s in self.backtest_symbols_var.get().split(",") if
                   s.strip()]
        ...
        if strategy == "turtle":
            strategy_params = {"entry_period": self.backtest_entry_period_var.get(),
                "exit_period": self.backtest_exit_period_var.get(),
                "atr_period": self.backtest_atr_period_var.get()}
        else:  # EMA
            strategy_params = {"fast_ema": self.backtest_fast_ema_var.get(),
        ...
        # Commando opbouwen
        cmd = [sys.executable, "-m", "src.analysis.backtest", "--strategy", strategy,
            "--timeframe", timeframe, "--period", period, "--initial-cash",
            initial_cash, "--symbols"]
        ...
        if strategy == "turtle":
            cmd.extend(
                ["--entry-period", strategy_params["entry_period"], "--exit-period",
                    strategy_params["exit_period"], "--atr-period",
                    strategy_params["atr_period"]])
        else:  # EMA
            cmd.extend(["--fast-ema", strategy_params["fast_ema"], "--slow-ema",
                strategy_params["slow_ema"], "--signal-ema",
        ...
        threading.Thread(target=self._run_process,
                         args=(cmd, self.backtest_output)).start()

    def run_optimization(self):
        """Voer optimalisatie uit met de ingestelde parameters."""
        # Optimalisatie parameters verzamelen
        strategy = self.optimize_strategy_var.get()
        symbols = [s.strip() for s in self.optimize_symbols_var.get().split(",") if
                   s.strip()]
        ...
        # Parameter ranges
        if strategy == "turtle":
            param_ranges = {"entry_period_range": self.optimize_entry_range_var.get(),
                "exit_period_range": self.optimize_exit_range_var.get(),
                "atr_period_range": self.optimize_atr_range_var.get()}
        else:  # EMA
            param_ranges = {"fast_ema_range": self.optimize_fast_ema_range_var.get(),
                "slow_ema_range": self.optimize_slow_ema_range_var.get(),
                "signal_ema_range": self.optimize_signal_ema_range_var.get()}

        # Commando opbouwen
        cmd = [sys.executable, "-m", "src.analysis.optimizer", "--strategy", strategy,
            "--timeframe", timeframe, "--period", period, "--metric", metric,
            "--max-combinations", max_combinations, "--symbols"]
        ...
        if strategy == "turtle":
            cmd.extend(["--entry-period-range", param_ranges["entry_period_range"],
                "--exit-period-range", param_ranges["exit_period_range"],
                "--atr-period-range", param_ranges["atr_period_range"]])
        else:  # EMA
            cmd.extend(
                ["--fast-ema-range", param_ranges["fast_ema_range"], "--slow-ema-range",
                    param_ranges["slow_ema_range"], "--signal-ema-range",
        ...
        # Uitvoeren in aparte thread
        threading.Thread(target=self._run_process,
                         args=(cmd, self.optimize_output)).start()
    ...
    def _run_process(self, cmd, output_widget):
        ...
        try:
            ...
            for line in process.stdout:
                self.root.after(0,
                                lambda l=line: self._append_to_output(output_widget, l))

            process.wait()

            if process.returncode == 0:
                self.root.after(0,
                                lambda: self.show_status("Proces succesvol voltooid"))
            else:
                self.root.after(0, lambda: self.show_status(
                    f"Proces voltooid met foutcode {process.returncode}"))

        except Exception as e:
            self.root.after(0, lambda: self._append_to_output(output_widget,
                f"\nFout bij uitvoeren: {e}\n"))
            self.root.after(0, lambda: self.show_status("Fout bij uitvoeren proces"))
    ...
    def open_folder(self, folder_name):
        ...
        if not os.path.exists(folder_path):
            try:
                ...
                messagebox.showerror("Map openen", f"Kon map niet aanmaken: {e}")
                return
