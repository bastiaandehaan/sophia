...
def setup_logging():
    ...
    log_file = os.path.join(log_dir,
                            f"optimize_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log")
...
def parse_arguments():
    ...
    parser.add_argument('--entry-period-range', type=str, default='10,20,30,40',
                        help='Range for entry_period (default: 10,20,30,40)')
    parser.add_argument('--exit-period-range', type=str, default='5,10,15,20',
                        help='Range for exit_period (default: 5,10,15,20)')
    parser.add_argument('--atr-period-range', type=str, default='10,14,20',
                        help='Range for atr_period (default: 10,14,20)')

    # Parameter ranges - EMA
    parser.add_argument('--fast-ema-range', type=str, default='5,9,12,15',
                        help='Range for fast_ema (default: 5,9,12,15)')
    parser.add_argument('--slow-ema-range', type=str, default='20,25,30',
                        help='Range for slow_ema (default: 20,25,30)')
    parser.add_argument('--signal-ema-range', type=str, default='5,7,9',
    ...
    parser.add_argument('--plot-top', type=int, default=5,
                        help='Number of top results to plot')
...
def generate_parameter_combinations(args, strategy: str, limit: int) -> List[
    Dict[str, Any]]:
    """
            ...
            for exit in exit_periods:
                for atr in atr_periods:
                    param_combinations.append(
                        {'entry_period': entry, 'exit_period': exit, 'atr_period': atr,
                            'risk_pct': 0.01,  # Fixed
...
def run_optimization(args, logger):
    ...
    logger.info(f"Generating parameter combinations for {args.strategy} strategy...")
    param_combinations = generate_parameter_combinations(args, args.strategy,
    ...
    print("\n" + "=" * 80)
    print(f"TOP {top_n} PARAMETER COMBINATIONS FOR {args.strategy.upper()} STRATEGY")
    print("=" * 80)
    ...
    for i, result in enumerate(results[:top_n]):
        ...
        if args.strategy == 'turtle':
            row = [i + 1, params['entry_period'], params['exit_period'],
                params['atr_period'], f"{metrics['total_return_pct']:.2f}%",
                f"{metrics['sharpe_ratio']:.2f}", f"{metrics['max_drawdown_pct']:.2f}%",
                f"{metrics['win_rate']:.2f}%", f"{metrics['profit_factor']:.2f}",
                metrics['total_trades']]
            headers = ["Rank", "Entry", "Exit", "ATR", "Return %", "Sharpe", "Drawdown",
                       "Win Rate", "Profit Factor", "Trades"]

        elif args.strategy == 'ema':
            row = [i + 1, params['fast_ema'], params['slow_ema'], params['signal_ema'],
                f"{metrics['total_return_pct']:.2f}%", f"{metrics['sharpe_ratio']:.2f}",
                f"{metrics['max_drawdown_pct']:.2f}%", f"{metrics['win_rate']:.2f}%",
                f"{metrics['profit_factor']:.2f}", metrics['total_trades']]
            headers = ["Rank", "Fast EMA", "Slow EMA", "Signal EMA", "Return %",
                       "Sharpe", "Drawdown", "Win Rate", "Profit Factor", "Trades"]
    ...
    if args.plot_top > 0:
        ...
        metrics_to_plot = [('total_return_pct', 'Total Return %'),
    ...
    with open(results_filename, 'w') as f:
        ...
        results_dict = {'strategy': args.strategy, 'timeframe': args.timeframe,
            'start_date': start_date, 'end_date': end_date, 'symbols': symbols,
            'metric': args.metric, 'results': [{'params': r['params'], 'metrics': {
                k: float(v) if isinstance(v, (np.float32, np.float64)) else v for k, v
                in r['metrics'].items()}} for r in results]}
