...
def test_strategy_using_connector(mock_connector, mock_risk_manager, logger_fixture):
...
@pytest.mark.integration
def test_complete_trading_workflow(mock_connector, mock_risk_manager, logger_fixture):
    ...
    mock_signal = {"signal": "BUY",
                   "meta": {"entry_price": entry_price, "stop_loss": stop_loss,
                            "atr": 0.005}, "timestamp": pd.Timestamp.now()}
    ...
    mock_connector.place_order.return_value = {"order_id": order_id, "symbol": symbol,
                                               "type": "BUY", "volume": position_size,
                                               "price": entry_price, "sl": stop_loss}
    ...
    if signal_result.get("signal") in ["BUY", "SELL"]:
        ...
        strategy.positions[symbol] = {"direction": signal, "entry_price": entry_price,
                                      ...
                                      "size": position_size,
                                      "entry_time": pd.Timestamp.now().strftime(
                                          "%Y-%m-%d %H:%M:%S")}
