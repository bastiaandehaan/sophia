...
WRAPPERS = {r.name: r.bytes for r in finder(distlib_package).iterator("") if
    r.name.endswith(".exe")}
...
class ScriptMaker(object):
    ...
    def __init__(self, source_dir, target_dir, add_launchers=True, dry_run=False,
                 fileop=None):
    ...
    def _get_shebang(self, encoding, post_interp=b'', options=None):
        ...
        if (
                sys.platform == 'cli' and '-X:Frames' not in post_interp and '-X:FullFrames' not in post_interp):  # pragma: no cover
    ...
    def _get_script_text(self, entry):
        return self.script_template % dict(module=entry.prefix,
            import_name=entry.suffix.split('.')[0], func=entry.suffix)
